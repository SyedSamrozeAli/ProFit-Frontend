...
function MemberPaymentTable({ paymentData, month, year }) {
  ...
  const column = [
    { name: "Member ID", selector: (row) => row.member_id, sortable: true, minWidth: '150px' },
    { name: "Member Name", selector: (row) => row.member_name, sortable: true, minWidth: '150px' },
    { name: "Payment Date", selector: (row) => row.payment_date, sortable: true, minWidth: '150px' },
    {
      ...
      minWidth: '80px',
      cell: (row) => (
        <span style={{ color: row.membership === 'Premium' ? '#FFD700' : '#00796B' }}>
          {row.membership}
        </span>
      ),
    },
    { name: "Payment Amount", selector: (row) => row.payment_amount, sortable: true, minWidth: '120px' },
    {
      name: "Balance",
      selector: (row) => row.balance,
      sortable: true,
      minWidth: '100px',
      cell: (row) => <span style={{ color: 'blue' }}>{row.balance}</span>,
    },
    {
      name: "Dues",
      ...
      sortable: true,
      minWidth: '100px',
      cell: (row) => <span style={{ color: 'red' }}>{row.dues}</span>,
    },
  ...
  const customStyles = {
    ...
    headCells: {
      style: { fontWeight: 'bold', borderBottom: '2px solid #E5E7EB', backgroundColor: '#F3F4F6' },
    },
  };
  ...
  const exportPDF = () => {
    const doc = new jsPDF();
    doc.text('Member Payment Data', 14, 16);
    doc.autoTable({
      head: [['Member ID', 'Member Name', 'Payment Date', 'Membership Type', 'Payment Amount', 'Balance', 'Dues', 'Payment Method', 'Payment Status']],
      body: data.map((row) => [
        row.member_id,
        row.member_name,
        row.payment_date,
    ...
    doc.save('MemberPaymentData.pdf');
  };
